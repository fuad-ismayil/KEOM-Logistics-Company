...
export default function AboutArea() {
    ...
    useEffect(() => {const observer = new IntersectionObserver((entries) => {entries.forEach((entry) => {if (entry.isIntersecting) {if (entry.target === mainImageRef.current) {setMainImageVisible(true);}if (entry.target === truckRef.current) {setTruckVisible(true);}observer.unobserve(entry.target);}});}, { threshold: 0 });if (mainImageRef.current) observer.observe(mainImageRef.current);if (truckRef.current) observer.observe(truckRef.current);return () => observer.disconnect();}, []);
    return (
                ...
                <div className="grid items-center gap-8 lg:gap-12 lg:grid-cols-2">
                    <div className="relative flex justify-center lg:block">
                        <div ref={mainImageRef} className={`relative overflow-hidden reveal-mask ${mainImageVisible ? 'animate' : ''}`}>
                            <Image src="/images/about/aboutarea/about_img_01.jpg" alt="Shipping" className="object-cover w-[342px] h-[410px] lg:w-[500px] lg:h-[600px]" priority width={500} height={600} />
