...
export default function Header() {
  ...
  return (
    <header className={`flex w-full justify-between items-center px-[24px] lg:px-[7.4%] z-50 left-0 transition-all duration-300 ${
      isScrolled ? "fixed top-0 bg-[#003B49] py-[20px] lg:py-1.5" : "absolute top-0 pt-[30px] lg:pt-[33px]"
    }`}>
                  ...
                  {({ open }) => (
                    <>
                      ...
                      <DisclosurePanel className="pb-4 flex flex-col">
                        {item.sub.map((s: any) => (
                          <Link key={s.n} href={s.h} className="text-[15px] text-[#333] uppercase pl-[10%] py-[10px] border-t border-gray-100 opacity-75" onClick={() => setMobileMenuOpen(false)}>{s.n}</Link>
                        ))}
                      </DisclosurePanel>
                    </>
                  )}
...
function DesktopMenu({ item }: { item: any }) {
  return (
    <Popover className="relative group">
      <PopoverButton className="focus:outline-none hover:text-[#FF7D44] transition-colors outline-none">
        {item.name}
      </PopoverButton>
      <div className="absolute top-full hidden group-hover:block pt-3">
        <div className="w-[220px] bg-white shadow-lg border-t-[4px] border-[#FF7D44] py-1">
          {item.sub.map((s: any) => (
            <Link key={s.h} href={s.h} className="block px-5 py-3 text-sm font-medium text-gray-700 hover:text-[#FF7D44] transition">{s.n}</Link>
          ))}
        </div>
      </div>
    </Popover>
