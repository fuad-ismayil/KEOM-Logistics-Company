...
type ServiceItem = { titleLine1: string; titleLine2: string; imgSrc: string; href: string; alt: string; };
...
export default function ServicesArea() {
  return (
    <section id="services-area2" className="relative overflow-hidden bg-white pt-[120px] pb-[90px]">
      <div className="mx-auto w-full max-w-[1284px]">
        <div className="grid grid-cols-1 justify-items-center gap-y-[30px] lg:grid-cols-3 lg:gap-x-[32px]">
          {SERVICES.map((item) => (
            <div key={`${item.titleLine1}-${item.titleLine2}`} className="mx-auto w-[85%] bg-white lg:w-[416px]">
              <div className="relative w-full aspect-[416/270]">
                <Image src={item.imgSrc} alt={item.alt} fill className="object-cover object-center" sizes="(min-width: 1024px) 416px, 85vw" />
              </div>
              <div className="relative flex h-[99px] items-center bg-white">
                ...
                <Link href={item.href} aria-label={`${item.titleLine1} ${item.titleLine2}`} className="absolute right-[30px] top-1/2 flex h-[50px] w-[50px] -translate-y-1/2 items-center justify-center rounded-full border-2 hover:text-[#FF7D44] border-[rgba(119,119,119,0.49)] hover:border-[#FF7D44] transition-all duration-300 text-[#777777] no-underline">
